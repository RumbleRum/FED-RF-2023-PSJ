...
/**************************************************** 
    ...
    -> wheel 이벤트
    -> 마우스 휠 작동시 발생!
    (이전 이벤트명 : mousewheel / DOMMiuseScroll(파폭))
...
function wheelFn(e){ // 이벤트 전달변수(자동)
    ...
    sts_wheel = 1;  // 잠금!!!!
    setTimeout(()=>{sts_wheel=0},500);  // -->>>>>>> 0.8초후 잠금 해제!!
    
    // 함수호출 확인
    // console.log('응나야!');

    // 1. 휠 방향에 따른 페이지변수 변경하기
    // 휠 방향은 wheelDelta 로 알아냄
    ...
    // 화면 단위로 이동하므로 윈도우 높이값을 기본값으로 처리
    // window.innerHeight -> 윈도우 높이값을 구해온다!!
...
// 메뉴그룹 배열만큼 클릭 기능 만들기 ///
// for of문 사용!
for(let x of menuGrp){ // x - gnbList, indicList 를 순회!
    x.forEach((ele,idx)=>{
        domFn.addEvt(ele,'click',()=>{
    
            // 1. 전역 페이지 변수 업데이트 하기
            pg_num = idx; // 메뉴 순번으로 업뎃!
            console.log('ㅋ',pg_num);
...
// 터치 시작 이벤트 호출
function touchStart(e){ /* e - 이벤트전달 변수 */
    // 모바일 이벤트 화면 위치값 구하기
    // 모바일 오리지널 이벤트 객체- originalEvent(제이쿼리에서 만 씀)
    // 하위 터치 이벤트 컬렉션 - touches[0]
    // 변경된 터치 이벤트 담는 컬렉션 - changedTouches[0]

    // 스크린 위치값 구하기
    // 제이쿼리는 originalEvent 사용함
...
// 터치 끝 이벤트 호출
function touchEnd(e){ /* e - 이벤트전달 변수 */
    // 모바일 이벤트 화면 위치값 구하기
    // 모바일 오리지널 이벤트 객체- originalEvent(제이쿼리에서 만 씀)
    // 하위 터치 이벤트 컬렉션 - touches[0]
    // 변경된 터치 이벤트 담는 컬렉션 - changedTouches[0]
...
// 이벤트 처리함수 : 화면이동 /////
function movePage(dir){ /// dir - 방향값 
    
    // // console.log('이동방향',dir);

     // 1. 페이지번호 반영하기 
     // 1(true)는 아랫방향, 0은 윗방향
     if(dir) pg_num++;
     else pg_num--;
 
     // 2. 페이지 한계수 체크(양끝 페이지 고정!)
     if(pg_num<0) pg_num=0;
     if(pg_num==total_pg) pg_num = total_pg-1;
