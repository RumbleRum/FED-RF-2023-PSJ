...
function setMod() {
    ...
    console.log("수정", optVal);

    // 만약 수정선택박스 값이 'show' 이면 돌아가!
    if (optVal == "show") return;

    // 1. 해당 idx의 값을 가지는 배열값을 선택함
    // 1. 로컬쓰 가져오기
    // 1-1.로컬쓰 데이터 가져오기 : minfo
    let orgData = localStorage.getItem("minfo");
...
function modifyData() {
    ...
    // 1-3 해당 아이디 찿기  : 배열 find() 메서드
    // find()로 찿아서 해당값을 직접 업데이트한다
    orgData.find(v=>{
        if(v.idx == selIdx){
            // 고유 idx값인 경우 입력값으로 업데이트하기

            // 선택 배열값인 객체의 제목과 내용을 다시넣고 업데이트하기
            // 다시넣고 업데이트하기
            v.tit = modTit.value;
