...
function EventShow() {
    ...
    const aladin = (lamp) => {    // lamp - 알라딘 주인공 이미지 경로
        ...
        ReactDOM.render(<MakeImg isrc={lamp} ialt="램프공주"/>,document.querySelector('#ala'));
    ...
    const getLamp = () => {
        ...
        // 1. 램프선택 : 컴포넌트 구성 요소에 넣음
        let lamp =  document.querySelector('.lamp');

        // 2. 램프에 이미지 넣기
        ReactDOM.render(<MakeImg isrc="https://cdn.011st.com/11dims/resize/600x600/quality/75/11src/product/3168457870/B.png" 
        ialt="알라딘 램프" info="false" />,lamp);
        ...
        <React.Fragment>
            <div id="tbox" style={{ textAlign: "center" }}>
                ...
                <img
                    ...
                    alt="지니"
                 onMouseOver={()=>{aladin('./images/ala3.jpg')}} />
            </div>
            {/* 램프가 들어갈 요소 */}
...
function moveCar() {
    ...
    // 번갈아서 왔다/갔다함
    car.style.transform = back?"translateX(150%) scale(2)":"translateX(0%) scale(1)";

    // 트랜지션 셋팅
    car.style.transition = "2s ease-in-out";

    // 번갈아서 1/0 전환 (비?집:놀이동산)
    back?back=0:back=1;
