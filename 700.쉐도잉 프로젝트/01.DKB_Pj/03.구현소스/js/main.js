...
// 모바일 검사 함수 /////////
const chkMob = () => {if($(window).width()<=1024)mob=1;
else mob=0;
console.log('모바일?',mob);

// 부가기능 : 모바일일때 기본 스타일 지우기
...
$('.ham').click(()=>{
  ...
  console.log('지금.header에 .on있나',$('.header').is('.on'));
...
$('.gnb li').click(function(){
  if(!mob) return; // 모바일이 아니면 나가!
  console.log('날클릭');
  // 서브메뉴 슬라이드 애니로 보이기/숨기기
  // 대상 : .smenu
  $(this).find('.smenu')
  .slideToggle(700,'easeInOutQuad') // 열거나 닫기
  .parent() // 부모로 올라감 li
  .siblings().find('.smenu') // 다른 li들 하위
  .slideUp(700,'easeInOutQuad') // 스르륵 닫힘 모두
...
brandBox.innerHTML = '';  /* 원래 브랜드 바로가기 내용은 날리기 */
...
// 내부의 option 데이터 요소는 배열데이터.map().join('')을 사용!
// 맵쬬인
// map() 메서드 리마인딩
// map()은 배열을 재구성하여 다시 같은 자리에 리턴하여
// 만들어지는 새로운 배열을 변수에 담거나 그자리에 리턴한다
