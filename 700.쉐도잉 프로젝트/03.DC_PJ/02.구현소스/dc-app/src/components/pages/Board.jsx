...
export function Board() {
  ...
  *************************************/
  const chgMode = (e) => {
    ...
    if(window.confirm('정말로 글을 삭제할꺼야?ㅠㅠ')){
  ...
  const compUsr = (usr) => {
    ...
    if (myCon.logSts !== null) {
      ...
      // 3. 로그인사용자 정보와 조회하기
      // 아이디로 조회함!
      if(cUser){  // 할당안되면 undefined 이므로 할당되었을때만 if문 처리
 ...
 /************************************* 
   ...
   * 조건 : 
        ...
        -> 방법 : 사용자가 방문한 글 고유번호를 배열에 기록하고
        조회하여 같은 글인경우 업데이트를 막아준다
        (이때 배열은 로컬스 세션스에 기록함! 이유는 브라우저 닫을 때 사라짐)
  ...
  *************************************/
    ...
    const plusCnt = () => {
      // 처음에 통과상태 설정하기
      let isOK = true;
      // 세션스에 등록된 글 or 로그인 사용자 글 일때 false 처리!

      // [현재 읽은 글은 cData.current로 읽어옴]
      let cidx = cData.current.idx;
      console.log('조회수 증가체크 idx',cidx);

      // [세션스에 cnt-idx 이름으로 배열 데이터를 저장함!]
      // 만약 없으면 우선 만들기 ////
      ...
      // [현재글 세션스에 처리하기] ///////
      if(isOK){ // 조회수 증가일 경우에만 글 번호 세션스 등록!
        // 세션스 배열에 idx값 넣기
